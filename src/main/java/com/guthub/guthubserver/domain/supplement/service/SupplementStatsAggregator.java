...
public class SupplementStatsAggregator {
    ...
    public void aggregateAndIndexAll() {
        ...
        // 🔹 1. 전체 집계는 단 한 번만 수행
        Map<Long, SupplementAggregateProjection> totalAggMap =
                reviewRepository.aggregatePerSupplementAll().stream()
                        .collect(Collectors.toMap(
                                SupplementAggregateProjection::getSupplementId,
                                a -> a
                        ));

        Map<Long, List<SupplementGutAggregateProjection>> gutAggMap =
                reviewRepository.aggregatePerSupplementByGutAll().stream()
        ...
        do {
            ...
            for (Long supplementId : page.getContent()) {
                ...
                List<SupplementGutAggregateProjection> gutAggs =
                        gutAggMap.getOrDefault(supplementId, Collections.emptyList());
    ...
    private SupplementDocument buildDocument(
        ...
        SupplementDocument.SupplementDocumentBuilder builder =
                SupplementDocument.builder()
                        .id(supplementId)
        ...
        if (agg != null && agg.getReviewCount() > 0) {
            builder
                    ...
                    .lastReviewDate(agg.getLastReviewAt())
                    .totalRankingScore(
